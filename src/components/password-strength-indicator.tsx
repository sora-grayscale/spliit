    ...
    return (
      <div className="space-y-2">
        <div className="flex items-center justify-between text-sm text-muted-foreground">
          ...
          <span>
            {SECURITY_CONSTANTS.MIN_PASSWORD_LENGTH}-
            {SECURITY_CONSTANTS.MAX_PASSWORD_LENGTH} characters
          </span>
        </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-2 text-xs text-muted-foreground">
            <span>
              {strength.length}/{SECURITY_CONSTANTS.MAX_PASSWORD_LENGTH}
            </span>
            {showDetails && (
              <Button
                type="button"
      ...
      {showDetails && (
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div
              className={`flex items-center gap-1 ${
                strength.hasNumbers ? 'text-green-600' : 'text-muted-foreground'
              }`}
            >
              <div
                className={`w-2 h-2 rounded-full ${
                  strength.hasNumbers ? 'bg-green-500' : 'bg-gray-300'
                }`}
              />
              Numbers
            </div>
            <div
              className={`flex items-center gap-1 ${
                strength.hasSymbols ? 'text-green-600' : 'text-muted-foreground'
              }`}
            >
              <div
                className={`w-2 h-2 rounded-full ${
                  strength.hasSymbols ? 'bg-green-500' : 'bg-gray-300'
                }`}
              />
              Symbols
            </div>
