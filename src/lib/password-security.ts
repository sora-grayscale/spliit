...
function calculateEntropy(password: string): number {
  const charSets = {
    lowercase: /[a-z]/.test(password)
      ? SECURITY_CONSTANTS.CHARSET_LOWERCASE
      : 0,
    uppercase: /[A-Z]/.test(password)
      ? SECURITY_CONSTANTS.CHARSET_UPPERCASE
      : 0,
    numbers: /[0-9]/.test(password) ? SECURITY_CONSTANTS.CHARSET_NUMBERS : 0,
    symbols: /[^A-Za-z0-9]/.test(password)
      ? SECURITY_CONSTANTS.CHARSET_SYMBOLS
      : 0,
  }
...
const WEAK_PATTERNS: ReadonlyArray<WeakPattern> = [
  ...
  {
    pattern:
      /qwerty|asdf|zxcv|1234|password|admin|user|test|guest|root|welcome|letmein/i,
    message: 'Contains common patterns',
    severity: 'high',
  },
...
function hasWeakPatterns(password: string): string[] {
  return WEAK_PATTERNS.filter(({ pattern }) => pattern.test(password)).map(
    ({ message }) => message,
  )
}
...
export function analyzePasswordStrength(password: string): PasswordStrength {
  ...
  if (length < SECURITY_CONSTANTS.MIN_PASSWORD_LENGTH) {
    feedback.push(
      `Password must be at least ${SECURITY_CONSTANTS.MIN_PASSWORD_LENGTH} characters`,
    )
  }
...
export function validatePassword(password: string): PasswordValidation {
  ...
  if (strength.length > SECURITY_CONSTANTS.MAX_PASSWORD_LENGTH) {
    errors.push(
      `Password must not exceed ${SECURITY_CONSTANTS.MAX_PASSWORD_LENGTH} characters`,
    )
  }
  ...
  if (missingCharTypes.length >= 3) {
    errors.push(
      `Password must contain at least one of: ${missingCharTypes.join(', ')}`,
    )
...
export function generateSecurePassword(length: number = 16): string {
  ...
  if (typeof length !== 'number' || !Number.isInteger(length) || length < 4) {
    throw new Error(
      'Password length must be a positive integer of at least 4 characters',
    )
  }

  if (length > SECURITY_CONSTANTS.MAX_PASSWORD_LENGTH) {
    throw new Error(
      `Password length cannot exceed ${SECURITY_CONSTANTS.MAX_PASSWORD_LENGTH} characters to prevent memory exhaustion attacks`,
    )
  }
